...
namespace MergeInc\Sort\Dependencies\DI\Compiler;
...
class Compiler
{
     ...
     * if PHP-MergeInc\Sort\Dependencies\DI configuration does not change.
     *
     * @var int
     */
    ...
    private function compileDefinition(string $entryName, Definition $definition) : string
    {
        ...
        switch (true) {
            ...
            case $definition instanceof StringDefinition:
                ...
                $code = 'return \MergeInc\Sort\Dependencies\DI\Definition\StringDefinition::resolveExpression(' . $entryName . ', ' . $expression . ', $this->delegateContainer);';
                break;
        ...
        if (!$isOptional) {
            throw new \MergeInc\Sort\Dependencies\DI\Definition\Exception\InvalidDefinition("The environment variable '{$definition->getVariableName()}' has not been defined");
        }
        return $defaultValue;
PHP;
                break;
    ...
    /**
     * @throws \MergeInc\Sort\Dependencies\DI\Definition\Exception\InvalidDefinition
     */
